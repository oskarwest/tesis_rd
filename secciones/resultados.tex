...
clasificación binaria}
	...
	\includegraphics[width=0.85\textwidth]{imágenes/resultados/confusion_binaria_original.png}
	\caption*{\textit{Fuente: Elaboración propia}}
\end{figure}
...
(15 y 15), aunque considerablemente mayor en total. ResNet50V2 e InceptionV3, en cambio,
presentan muchos más errores en total (24 falsos negativos y 24 falsos positivos, y 30 y
30, respectivamente): a diferencia de MobileNetV2, que concentra sus pocos errores
mayoritariamente en falsos positivos (el tipo de error clínicamente menos costoso), estas
dos redes no muestran ningún sesgo protector hacia la detección de RD, lo que
constituye un patrón menos favorable para una herramienta de tamizaje y refuerza, junto
con las métricas agregadas de la Tabla~\ref{tab:comparacion_redes}, la decisión de
descartar ambas arquitecturas para los experimentos posteriores.
...
positivos en MobileNetV2 (aproximadamente 5 vs. 6 sin filtrado), lo que mejora la
especificidad clínica y disminuye derivaciones innecesarias. Los falsos negativos
también se redujeron ligeramente (4 vs. 5), lo cual es clínicamente relevante: implica
...
reducción de falsos positivos señalada anteriormente (de 6 a 5 casos), mientras que en
...
filtrado también reduce, aunque de forma más modesta, el número total de errores en
ResNet50V2 e InceptionV3, manteniendo en ambas arquitecturas un balance similar entre
falsos negativos y falsos positivos: en ResNet50V2 ambos bajan de 24 a 20, y en
InceptionV3 de 30 a 27. A diferencia de MobileNetV2, que concentra sus errores
mayoritariamente en falsos positivos, estas dos redes no muestran ese sesgo protector
hacia la detección de RD ni antes ni después del filtrado, lo que confirma que, además
de mejorar el desempeño agregado, el preprocesamiento no modifica el patrón de error de
cada arquitectura, solo reduce su magnitud total.
...
con filtrado}
	...
	\includegraphics[width=0.85\textwidth]{imágenes/resultados/confusion_binaria_filtrado.png}
	\caption*{\textit{Fuente: Elaboración propia}}
\end{figure}
...
0.96 con filtrado (Tabla~\ref{tab:comparacion-umbral}). El modelo muestra un balance
adecuado entre la detección de casos positivos y la reducción de falsos positivos, lo que
lo hace apropiado para tareas de tamizaje. Con este umbral, y considerando una
prevalencia de RD del 50\% en la población examinada, por cada 1,000 pacientes se
esperarían aproximadamente: 480 verdaderos positivos y 480 verdaderos negativos, 20
falsos negativos (casos no detectados de RD) y 20 falsos positivos (pacientes sin RD que
...
Tabla~\ref{tab:validacion-externa}, MESSIDOR-2 mostró los mejores resultados (F1-score de 0.91), reflejando su
alta calidad de imágenes y protocolo de adquisición estandarizado. FOSCAL, como dataset
clínico realista proveniente de un centro colombiano, exhibió resultados comparables
(F1-score de 0.89) que reflejan condiciones de práctica clínica habitual. IDRiD presentó
métricas intermedias (F1-score de 0.85) debido a su tamaño reducido y mayor
heterogeneidad.
...
datasets externos}
	...
	\includegraphics[width=0.85\textwidth]{imágenes/resultados/confusion_binaria_externa.png}
	\caption*{\textit{Fuente: Elaboración propia}}
\end{figure}
...
FOSCAL & 0.90 & 0.95 & 0.87 & 0.89 \\
IDRiD & 0.82 & 0.84 & 0.88 & 0.85 \\
MESSIDOR-2 & 0.92 & 0.92 & 0.89 & 0.91 \\
\midrule
\textbf{Promedio} & \textbf{0.88} & \textbf{0.90} & \textbf{0.88} & \textbf{0.88} \\
\bottomrule
\end{tabularx}
\caption*{\textit{Fuente: Elaboración propia}}
\end{table}
...
Gulshan et al. (2016) & Inception V3 personalizada & EyePACS + Messidor-2 & 0.975 \\
...
dataset. Si bien Gulshan et al. reportaron métricas ligeramente superiores (97.5\%
...
propuesto logra resultados comparables con un conjunto de datos considerablemente más
limitado (2,100 imágenes: 1,050 por clase), lo cual sugiere que la estrategia de
preprocesamiento y las técnicas de regularización implementadas compensan parcialmente
las limitaciones en volumen de datos.
...
\begin{figure}[H]
	\centering
	\caption{Matrices de confusión para la evaluación de cada clasificador multiclase usando el
	dataset de Kaggle RGB}
...
únicamente el canal verde como entrada; el lector puede comparar directamente estas
matrices contra las de la Figura~\ref{fig:confusion-multiclase-rgb} para observar el efecto
del canal sobre la distribución de aciertos y errores de cada arquitectura.

% TODO (Camilo): se actualizó SOLO la matriz de MobileNetV2 (panel a) en esta figura
% con los nuevos valores que compartiste (matriz_confusion_organizada_2.xlsx, "Matriz 1").
% Los siguientes elementos del documento todavía reflejan la matriz ANTERIOR de
% MobileNetV2-verde y quedan pendientes de recalcular para que sean consistentes con
% este cambio: la fila "MobileNetV2" del canal Verde en la Tabla de métricas agregadas
% (sec:metricas-multiclase-canales / tab:metricas-multiclase-canales), la fila
% "MobileNetV2" de la Tabla 19 (tab:mejora-canal-verde), y las cifras puntuales de
% sensibilidad de MobileNetV2-verde citadas en la prosa de esta sección y en discusion.tex
% (p. ej. "de 0.51 (RGB) a 0.57" para la clase Severa). Falta decidir y actualizar esos
% valores.
\begin{figure}[H]
	\centering
	\caption{Matrices de confusión para la evaluación de cada clasificador multiclase usando el
	dataset de Kaggle canal verde}
...
\begin{figure}[H]
	\centering
	\caption{Matrices de confusión para la evaluación de cada clasificador multiclase usando el
	dataset de Kaggle canal rojo}
	\label{fig:confusion-multiclase-rojo}
	\includegraphics[width=\textwidth]{imágenes/resultados/confusion_multiclase_rojo.png}
	\caption*{\textit{Fuente: Elaboración propia}}
\end{figure}

Más allá de comparar los aciertos en la diagonal principal, las matrices de confusión
permiten caracterizar la naturaleza de los errores cometidos por cada modelo,
distinguiendo entre confusiones clínicamente esperables (entre categorías de severidad
adyacentes, cuyo límite diagnóstico es ambiguo incluso para observadores humanos) y
errores clínicamente graves, en los que el modelo asigna una categoría alejada varios
niveles de la gravedad real. Codificando las cinco clases de forma ordinal (Normal=0,
Leve=1, Moderada=2, Severa=3, RDP=4) y sumando los errores de las cuatro arquitecturas
para cada canal, la Tabla~\ref{tab:tipos-error-confusion} resume esta distinción.

\begin{table}[H]
	\centering
	\caption{Distribución de errores de clasificación multiclase según distancia de
	severidad, agregada en las cuatro arquitecturas}
	\label{tab:tipos-error-confusion}
	\begin{tabularx}{\textwidth}{l Y Y Y Y}
		\toprule
		Configuración & Adyacentes & No adyacentes & Referible $\rightarrow$ No referible & Severa/RDP $\rightarrow$ Normal/Leve \\
		\midrule
		RGB   & 57.4\% & 42.6\% & 38.6\% & 14.3\% \\
		Verde & 56.8\% & 43.2\% & 37.0\% & 12.5\% \\
		Azul  & 50.3\% & 49.7\% & 41.1\% & 18.6\% \\
		Rojo  & 54.7\% & 45.3\% & 40.9\% & 16.0\% \\
		\bottomrule
	\end{tabularx}
	
	\caption*{\textit{Fuente: Elaboración propia. Porcentajes calculados sobre el total de
	errores (celdas fuera de la diagonal) sumando las matrices de confusión de las cuatro
	arquitecturas}}
\end{table}

En las cuatro configuraciones evaluadas, entre el 50\% y el 57\% de los errores
corresponden a confusiones entre clases adyacentes (por ejemplo, Normal-Leve o
Moderada-Severa), un patrón esperable dado el carácter progresivo y continuo de la
retinopatía diabética, en el cual los casos en el límite entre dos etapas consecutivas son
inherentemente más difíciles de separar. El resto de los errores (43\%-50\%) implica saltos
de dos o más niveles de severidad. Dentro de este grupo, el subconjunto clínicamente más
relevante es aquel en el que un caso de RD referible (Moderada, Severa o RDP) se clasifica
como no referible (Normal o Leve), pues equivale a omitir un caso que requeriría derivación
a oftalmología: este tipo de error representa entre el 37\% (canal verde) y el 41\% (canal
azul) del total de errores, es decir, aproximadamente 4 de cada 10 errores del modelo.

Un subconjunto aún más crítico dentro de esta categoría es aquel en el que casos Severos o
de RDP (las etapas con mayor riesgo de pérdida visual) se clasifican directamente como
Normal o Leve. Este tipo de error representa el 12.5\% de los errores totales con el canal
verde, frente a 14.3\% con RGB, 16.0\% con el canal rojo y 18.6\% con el canal azul, lo cual
es consistente con la mayor sensibilidad del canal verde para la clase Severa cuantificada
más adelante en esta misma sección. En el extremo opuesto, las falsas alarmas
(casos Normales clasificados como Severa o RDP) son poco frecuentes en las cuatro
configuraciones (entre 0.5\% y 1.7\% de los errores), lo cual indica que el sesgo de los
errores del modelo tiende hacia la subestimación de la severidad más que hacia la
sobreestimación.

Cabe destacar que gran parte de la proporción de errores no adyacentes se explica por el
comportamiento de VGG19: como se observó en las matrices de confusión (Figura~
\ref{fig:confusion-multiclase-azul}, panel d, y Figura~\ref{fig:confusion-multiclase-rojo},
panel d), esta arquitectura colapsa hacia las clases Normal, Leve y RDP con los canales
rojo y azul, sin predecir nunca Moderada ni Severa, lo que concentra artificialmente sus
errores en saltos de severidad amplios y no necesariamente refleja un patrón generalizable
a las demás arquitecturas.
...
\begin{table}[H]
	...
	retinopatía diabética usando todos y cada uno de los canales RGB}
...
Cabe señalar que esta ventaja no se extiende de forma uniforme a la clase RDP (RD4): en
