...
\textbf{Capa convolucional:} Es la primera capa de cada modelo CNN. Una capa
convolucional tiene varios filtros de tamaño $M \times M$, cada uno de los cuales realiza
la operación de convolución. El producto escalar se toma entre el filtro y las partes
cubiertas por el filtro en la imagen de entrada, deslizando el filtro sobre la imagen. El
resultado se denomina mapa de características. A través de este proceso se pueden
identificar esquinas y bordes. Más adelante, este mapa de características se traslada y
se alimenta a otras capas para aprender características más complejas que no se
...
\textbf{Regularización L1 y L2:} La regularización L2 consiste en agregar un término de
penalización a la función de pérdida, proporcional a la suma de los cuadrados de los
pesos de la red:
...
donde $\lambda$ es el parámetro que controla la magnitud de la penalización. A diferencia
de L1, la regularización L2 no lleva los coeficientes a cero, lo que contribuye a una
mayor estabilidad numérica del modelo. En \myfootcite{kukavcka2017regularization} se
presenta una taxonomía de métodos de regularización en el aprendizaje profundo,
destacando la efectividad de L2 en la mejora de la capacidad de generalización de las
redes neuronales.
