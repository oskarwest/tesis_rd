\chapter{MARCO TEÓRICO} \label{chap:fundamentos}
...
sustentan el presente trabajo, siguiendo una progresión desde el problema clínico hacia
la solución computacional: primero, se expone la fisiopatología de la retinopatía
diabética; segundo, se describe la naturaleza y características de las imágenes de fondo
de ojo, la modalidad utilizada para observar dichas manifestaciones, así como la
clasificación clínica de la RD a partir de estas imágenes; y tercero, se desarrollan los
conceptos fundamentales de inteligencia artificial, aprendizaje automático y redes
neuronales convolucionales que constituyen la base metodológica del trabajo.
...
como se muestra en la Figura~\ref{fig:fundus-image}.
...
evaluar el grosor y detectar edema macular. Sin embargo, la incorporación de la OCT a
programas de tamizaje a gran escala se ha visto limitada por el alto costo de los
equipos y la necesidad de personal técnico especializado para su operación
\myfootcite{wong2020oct}, mientras que las cámaras de fondo de ojo, incluyendo
variantes portátiles y de bajo costo basadas en adaptadores para smartphones, han
demostrado ser una alternativa viable para expandir la cobertura de tamizaje hacia
zonas con acceso limitado a especialistas \myfootcite{aboutaha2024screening}. Por esta
razón, las imágenes del fondo de ojo son las más utilizadas en programas de detección
masiva de RD: además de proporcionar una visualización directa de la retina, donde se
producen los cambios patológicos característicos de la enfermedad, su menor costo de
adquisición, mayor disponibilidad y portabilidad las hacen más adecuadas que la OCT
para el escenario de acceso limitado a especialistas planteado en el
Capítulo~\ref{chap:planteamiento-problema}. Con el avance de la inteligencia
artificial, estas imágenes pueden además ser analizadas automáticamente mediante
algoritmos de aprendizaje profundo, permitiendo un tamizaje masivo y eficiente.
...
La detección y el diagnóstico oportuno son fundamentales, ya que permiten la
intervención antes de que la enfermedad progrese: brindan una mayor oportunidad para un
tratamiento eficaz, reducen el riesgo de ceguera y atenúan las complicaciones asociadas a
la retinopatía diabética. Un control adecuado de los niveles de glucosa en sangre, sumado
a la detección temprana, puede retrasar o incluso evitar el avance hacia formas más
graves de retinopatía. Además, estudios han demostrado que los pacientes que se someten a
exámenes regulares de fondo de ojo tienen una mejor tasa de preservación de la visión a
largo plazo \myfootcite{cusick2005associations}.
...
RDP (Figura~\ref{fig:etapas-rd}d), los nuevos vasos
...
de visión. La Tabla~\ref{tab:etapas-rd} resume, para cada una de estas cinco etapas, los
hallazgos específicos que un especialista (o un modelo automatizado) debe identificar en
la imagen de fondo de ojo para clasificarla correctamente.

\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.5}
\caption{Etapas de la retinopatía diabética y sus manifestaciones clínicas en imágenes de
fondo de ojo}
\label{tab:etapas-rd}
\begin{tabularx}{\textwidth}{X X}
\hline
\textbf{Etapa} & \textbf{Hallazgos en la imagen de fondo de ojo} \\ \hline
...
\end{tabularx}
\caption*{\textit{Fuente: Adaptado de la clasificación clínica estándar de retinopatía diabética}}
\end{table}
...
datos de entrada y salida, pero el algoritmo debe aprender por sí mismo las reglas que
relacionan ambos conjuntos. Esto permite que el programa sea capaz de aprender patrones
y relaciones subyacentes, incluidas relaciones no lineales, entre las variables o
características de los datos y, con el tiempo, adaptar su comportamiento,
...
Dentro del campo del aprendizaje automático, existen diferentes enfoques según la
estructura de los datos disponibles y el tipo de tarea a resolver: el aprendizaje
supervisado, que utiliza datos etiquetados para aprender a mapear entradas a salidas
conocidas; el no supervisado, que busca patrones ocultos en datos sin etiquetar, como
agrupamientos (clustering) o reducción de dimensionalidad; el semisupervisado, que
combina datos etiquetados y no etiquetados cuando etiquetar grandes volúmenes de datos es
costoso o impráctico; y el aprendizaje por refuerzo, en el que un agente aprende a tomar
decisiones secuenciales a partir de recompensas y penalizaciones recibidas de un entorno
\myfootcite{jordan2015machine}.

Este trabajo se enmarca en el aprendizaje supervisado, ya que se dispone de un conjunto
de imágenes de fondo de ojo etiquetadas tanto por la presencia o ausencia de retinopatía
diabética como por su nivel de severidad, en una escala de 0 a 4. En el aprendizaje supervisado, el modelo se entrena
utilizando un conjunto de datos para el cual ya se conoce la salida correcta (la
etiqueta), con el objetivo de aprender una función que mapee nuevas entradas, en este
caso nuevas imágenes a su categoría correspondiente. Los modelos de clasificación,
como los desarrollados en este proyecto, son un caso típico de este enfoque: a partir de
las características aprendidas durante el entrenamiento, el modelo asigna a cada imagen
la categoría que mejor corresponde a su contenido.

\subsection{Redes neuronales artificiales}

Las redes neuronales artificiales (RNA) son modelos computacionales inspirados, de
manera simplificada, en el funcionamiento del cerebro humano. Su unidad básica es la
neurona artificial, la cual recibe uno o varios valores de entrada, cada uno multiplicado
por un peso que representa la importancia relativa de esa entrada, y suma un término
adicional llamado sesgo (bias). El resultado de esta combinación lineal se transforma
mediante una función de activación, que introduce no linealidad en el modelo y determina
el valor de salida de la neurona \myfootcite{lecun2015deep}.

Una red neuronal se construye organizando estas neuronas en capas: una capa de entrada,
que recibe los datos originales; una o varias capas ocultas, en las que se realizan
transformaciones sucesivas de la información; y una capa de salida, que produce el
resultado final (por ejemplo, la clase predicha). Cuantas más capas ocultas tenga la red,
mayor es su capacidad para representar relaciones complejas entre las variables de
entrada, lo que da origen al término aprendizaje profundo.

El entrenamiento de una red neuronal consiste en ajustar iterativamente los pesos de
todas sus conexiones para que las predicciones del modelo se acerquen lo más posible a
las salidas reales. Para ello se define una función de pérdida (o función de costo), que
cuantifica la diferencia entre la predicción de la red y la etiqueta verdadera; entre
menor sea esta diferencia, mejor es el desempeño del modelo. El proceso de entrenamiento
busca minimizar dicha función mediante algoritmos de optimización basados en el
gradiente, ajustando los pesos en la dirección que reduce el error
\myfootcite{lecun2015deep}. Las redes neuronales convolucionales, presentadas a
continuación, son una especialización de las redes neuronales artificiales diseñada
específicamente para aprovechar la estructura espacial presente en las imágenes.
...
las porciones de la imagen con las que coincide. El filtro \enquote{captura} patrones locales,
...
que contribuye al proceso de extracción de características y toma de decisiones. La
Figura~\ref{fig:cnn-general} ilustra esta estructura general, mostrando cómo la imagen de
entrada atraviesa sucesivas capas convolucionales y de agrupamiento antes de llegar a las
capas completamente conectadas que producen la clasificación final.
...
que aprenden rasgos más complejos \myfootcite{purwono2022understanding}. La
Figura~\ref{fig:convolution} ejemplifica este proceso, mostrando cómo un kernel de
$3 \times 3$ se desliza sobre la imagen de entrada y genera, mediante productos punto
sucesivos, el mapa de características correspondiente.
...
La Figura~\ref{fig:maxpooling} ejemplifica la operación de max-pooling, donde para cada
región de $2 \times 2$ del mapa de características se conserva únicamente el valor
máximo, reduciendo a la mitad sus dimensiones espaciales.
...
componen de varias \enquote{cabezas de atención}, por lo que es posible observar los datos desde
...
estocástico aplicado a las características internas de la red; su papel como técnica de
regularización se describe en la siguiente subsección.
...
incluyendo su ruido y sus valores atípicos, lo que se conoce como \enquote{aprendizaje de
...
subyacentes en los datos. La Figura~\ref{fig:overfitting} ilustra estos tres escenarios:
el subajuste, donde el modelo ni siquiera captura la tendencia de los datos de
entrenamiento; el ajuste adecuado, en el que la curva aprendida generaliza bien a datos
nuevos; y el sobreajuste, en el que el modelo se ajusta excesivamente al ruido de los
datos de entrenamiento a costa de su desempeño en datos no vistos.
...
Los modelos de aprendizaje profundo, especialmente las CNN, son especialmente susceptibles al sobreajuste debido a su alta capacidad y su
habilidad para aprender patrones detallados en datos a gran escala. A continuación se
describen las técnicas de regularización efectivamente utilizadas en este trabajo, el
riesgo específico que cada una busca controlar y su pertinencia en el contexto de
imágenes médicas.
...
de entrenamiento. Esta aleatoriedad es particularmente útil en las capas densas
superiores de este trabajo, que contienen numerosos parámetros nuevos entrenados con un
conjunto de datos etiquetados limitado frente al tamaño típico de ImageNet.

\textbf{Regularización L2:} Consiste en agregar a la función de pérdida un término de
penalización proporcional a la suma de los cuadrados de los pesos de la red:

\begin{equation}
L_{total} = L_{original} + \lambda \sum_{i} w_i^2
\end{equation}

Esta penalización reduce la magnitud de los pesos sin llevarlos exactamente a cero, lo
que distribuye la importancia entre características correlacionadas y contribuye a una
mayor estabilidad numérica del entrenamiento; $\lambda$ es el hiperparámetro que controla
la magnitud de la penalización. En \myfootcite{kukavcka2017regularization} se presenta
una taxonomía de métodos de regularización en el aprendizaje profundo, destacando la
efectividad de esta técnica en la mejora de la capacidad de generalización de las redes
neuronales, especialmente relevante dado el desbalance y la variabilidad presentes en el
conjunto de datos de retinopatía diabética utilizado.
...
detenerlo cuando dicho rendimiento deja de mejorar. Así se evita que el modelo continúe
ajustándose a particularidades del conjunto de entrenamiento una vez que ha dejado de
aprender patrones generalizables, evitando además entrenamientos innecesariamente largos.
Este método ha sido probado en distintas investigaciones \myfootcite{brownlee2018gentle},
donde se demuestra que el early stopping puede reducir el sobreajuste y mejorar la
generalización de las redes neuronales profundas.

\textbf{Capas GaussianNoise:} Como se describió en la subsección anterior, estas capas
inyectan ruido gaussiano sobre las activaciones durante el entrenamiento. Esto actúa como
regularizador, al exponer al modelo a versiones ligeramente corruptas de las
representaciones aprendidas y forzar al clasificador a aprender patrones robustos y no
dependientes de valores exactos de activación. Resulta especialmente pertinente en este
trabajo por dos razones: las capas de clasificación añadidas sobre los backbones
preentrenados contienen muchos parámetros nuevos y son propensas a sobreajustarse cuando
los datos etiquetados disponibles son limitados, y la inyección de ruido simula la
variabilidad propia de la adquisición de imágenes de fondo de ojo en condiciones clínicas
reales (ruido del sensor, diferencias de iluminación o preprocesado). Estudios empíricos
en problemas clínicos y de visión han mostrado mejoras en métricas de generalización y
AUC cuando se usa inyección de ruido \myfootcite{zur2009noise}.
...
de entrada. Esta técnica resulta particularmente útil en este trabajo para mitigar el
desbalance entre clases de severidad de RD, donde etiquetar nuevas imágenes es costoso,
al enriquecer artificialmente la representación de las clases minoritarias sin requerir
datos adicionales.
...
\section{ARQUITECTURAS DE REDES NEURONALES CONVOLUCIONALES UTILIZADAS} \label{sec:arquitecturas-cnn}
...
profundas de CNN, han demostrado ser eficaces en
...
activaciones no lineales después de la reducción de dimensiones. La
Figura~\ref{fig:mobilenetv2} muestra la arquitectura completa, donde pueden identificarse
los bloques residuales invertidos y la secuencia de expansión, convolución por
profundidad y proyección descrita a continuación.

\begin{figure}[H]
	\centering
	\caption{Arquitectura de MobileNetV2 mostrando bloques residuales invertidos}
	\label{fig:mobilenetv2}
	\includegraphics[width=0.8\textwidth]{imágenes/metodología/arq_mobilenetv2_font.png}
...
profundas. La Figura~\ref{fig:resnet50v2} muestra la organización de estos bloques
residuales con pre-activación a lo largo de las 50 capas de la red.

\begin{figure}[H]
	\centering
	\caption{Arquitectura de ResNet50V2 con bloques residuales mejorados}
	\label{fig:resnet50v2}
	\includegraphics[width=1\textwidth]{imágenes/metodología/arq_resnet50v2_font.png}
...
entrenamiento, mejorando la convergencia y la precisión del modelo. La
Figura~\ref{fig:inceptionv3} muestra la disposición de los módulos Inception en paralelo,
donde se aprecia cómo múltiples ramas de convolución y pooling procesan simultáneamente
la misma entrada.
...
\begin{figure}[H]
	...
	\includegraphics[width=1\textwidth]{imágenes/metodología/arq_inceptionv3_font.png}
...
retinopatía diabética, clasificación de imágenes médicas. La Figura~\ref{fig:vgg19}
muestra esta organización secuencial de bloques convolucionales de filtros $3 \times 3$,
seguidos de capas de pooling y de las capas completamente conectadas finales.

\begin{figure}[H]
	\centering
	\caption{Arquitectura de VGG19 con bloques convolucionales secuenciales}
	\label{fig:vgg19}
	\includegraphics[width=1\textwidth]{imágenes/metodología/arq_vgg19_font.png}
