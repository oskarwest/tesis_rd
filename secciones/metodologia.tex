...
Una etapa clave en la construcción de una CNN es la 
...
\enquote{derecha} (por 
ejemplo: 1\_izquierda.jpg). Cada imagen fue evaluada por un especialista
...
principales de este estudio es desarrollar algoritmos capaces de enfrentar
...
Este conjunto cuenta con: etiquetas de gradación de RD en una 
...
anomalías presentes, incluyendo el nivel de RD, lo que permitió 
...
Cabe precisar que la Tabla~\ref{tab:distribucion_clases} organiza las imágenes según los
cinco niveles de severidad empleados en la tarea multiclase. Para la clasificación
binaria, las cuatro categorías con presencia de la enfermedad (Leve, Moderada, Severa y
Proliferativa) se agrupan en una sola clase RD, frente a la clase No RD; la
Sección~\ref{subsec:balanceo-clases} detalla cómo se construyó, a partir de esta
agrupación, el conjunto de entrenamiento del clasificador binario a partir del dataset de
Kaggle. En los tres conjuntos utilizados para validación externa (MESSIDOR-2, IDRiD y
FOSCAL), en cambio, la versión binaria se obtuvo directamente a partir de los conteos de
la Tabla~\ref{tab:distribucion_clases}: se conservó la cantidad de imágenes No RD
indicada para cada dataset, y se sumaron las demás columnas de severidad para conformar
la clase RD, sin aplicar submuestreo ni sobremuestreo sobre estos conjuntos.

\subsection{División de datos para entrenamiento y validación} \label{subsec:division-datos}

Para la clasificación multiclase, el dataset de Kaggle se dividió en tres subconjuntos
siguiendo una estrategia de partición estratificada. Esta elección responde directamente al marcado desbalance de
...
Esta división se implementó utilizando la función train\_test\_split de
scikit-learn con el parámetro stratify para asegurar que la distribución de
...
imágenes originales sin duplicar; el balanceo de clases mediante submuestreo y aumento de
datos (Sección~\ref{subsec:balanceo-clases}) se aplica \textbf{después}, y únicamente
sobre el conjunto de entrenamiento resultante. Los conjuntos de validación y prueba
conservan la distribución de clases real del dataset, sin balancear, de manera que las
métricas reportadas reflejan el desempeño del modelo sobre datos no vistos durante el
entrenamiento y con una proporción de clases representativa del problema original. Esta
división estratificada corresponde específicamente al escenario multiclase; la
construcción del conjunto de entrenamiento del clasificador binario, que no parte de esta
partición, se describe en la Sección~\ref{subsec:balanceo-clases}.
...
En el clasificador binario, en cambio, no se aplican las técnicas de balanceo descritas
anteriormente para el escenario multiclase (submuestreo y aumento de datos). El enfoque
consistió en agrupar primero todas las categorías con retinopatía en una sola clase (RD)
frente a las imágenes sin enfermedad (No RD), y extraer después, aleatoriamente del
dataset de Kaggle, un total de 1,050 imágenes de la clase No RD y 1,050 imágenes de la
clase RD (estas últimas seleccionadas aleatoriamente entre las cuatro categorías de
severidad restantes). Esta construcción produce, por diseño, una proporción 50:50 entre
ambas clases, sin necesidad de aplicar sobremuestreo ni submuestreo adicional. Este total
de 2,100 imágenes se dividió mediante partición estratificada en entrenamiento (80\%;
840 imágenes por clase), validación (10\%; 105 imágenes por clase) y prueba (10\%; 105
imágenes por clase), siguiendo el mismo principio de partición antes de cualquier
procesamiento adicional descrito en la Sección~\ref{subsec:division-datos} para el
escenario multiclase. El conjunto de validación se utilizó para optimizar el umbral de
decisión (Sección~\ref{subsec:umbral-decision}), y el de prueba, de 105 imágenes por
clase, es el que se reporta en las matrices de confusión del Capítulo~\ref{chap:resultados}.
La construcción de los conjuntos de validación externa para
este clasificador, que tampoco se balancean, se describe en la
Sección~\ref{sec:conjuntos-datos}.
...
ImageDataGenerator de Keras, que permitió generar nuevas muestras a partir de 
...
La suite TensorFlow Extended incluye módulos para aumento de datos (tf.image) y 
...
centró en el desarrollo de un modelo para la detección binaria de RD,
discriminando entre casos positivos (presencia de la enfermedad) y negativos 
...
varias capas densas intercaladas con GaussianNoise y regularización (Dropout y L2), hasta
...
Adicionalmente, se calculó el AUC-ROC, que proporciona una
